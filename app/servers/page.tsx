...
export default function ServersPage() {
  ...
  const handleCreateServer = async (serverData: Omit<Server, "id" | "createdAt">) => {
    try {
      await api.post("api/servers", serverData);
      await refreshData();
    } catch (err) {
  ...
  const handleUpdateServer = async (id: string, serverData: Omit<Server, "id" | "createdAt">) => {
    try {
      await api.put(`api/servers/${id}`, serverData);
      await refreshData();
    } catch (err) {
  ...
  return (
    <ProtectedRoute>
      <Paper p="md" style={{ boxShadow: "0 0.5em 1em -0.125em rgba(10,10,10,0.1), 0 0 0 1px rgba(10,10,10,0.02)" }}>
        <Stack gap="lg">
          <Title order={2}>Servers</Title>
          <ServersTable
            data={servers}
