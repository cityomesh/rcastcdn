...
export const ServersTable = ({
  ...
  const columnDefs: ColDef<Server>[] = [
    ...
    },
    {
      ...
      width: 150,
      sortable: true,
      valueGetter: (params: ValueGetterParams<Server>) =>
  ...
  const handleApplySSHKeyRestart = async () => {
    ...
    try {
      ...
      // ఇక్కడ మేము కేవలం "executed successfully" messages మాత్రమే చూపిస్తాం
      const successMessages = results
        .map((res) => (res.success ? "✅ executed successfully" : res.error || "Failed"))
        .join("\n");
