...
//   const [selectedServer, setSelectedServer] = useState<string>("All Servers"); // 🆕 new state
...
// // 🧩 Clean + unique IP extraction logic
// const extractIp = (displayName: string) => {
//   // Extract only the actual IP part (ignore anything in parentheses)
//   const match = displayName.match(/\b\d{1,3}(?:\.\d{1,3}){3}\b/);
//   return match ? match[0].trim() : displayName.trim();
// };

// // 🆕 Create a unique set of pure IPs
// const uniqueServers = Array.from(
//   new Set(
//     routeServers.flatMap((item) =>
//       item.servers.map((s) => extractIp(s.displayName))
//     )
//   )
// ).filter((ip) => ip && ip !== "");
...
// import { api } from "@/app/utils/api";
// import { IconSearch, IconRefresh, IconFilter } from "@tabler/icons-react";
// import { StreamType, RouteServerAssignment, Server } from "@/app/types/server";
...
//   useEffect(() => {
//     (window as any).SELECTED_SERVER = selectedServer;
//   }, [selectedServer]);
...
//   // Unique server IPs for dropdown
//   const uniqueServers = Array.from(
...
//   // Flatten data so each server gets a separate row
//   const flattenedData = filteredData.flatMap((item) =>
//     item.servers.map((server) => ({
//       ...item,
//       singleServer: server, // new property
//     }))
//   );
